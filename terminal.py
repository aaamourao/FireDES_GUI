...
from PyQt5.QtWidgets import QAbstractScrollArea, QFrame, QSizePolicy
...
class Terminal(QAbstractScrollArea):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setGeometry(6, 517, 995, 200)
    ...
    def event(self, event):
        t = event.type()
        if t == QEvent.KeyPress and (event.key == Qt.Key_Tab or event.key() == \
                Qt.Key_Backtab):
            self.keyPressEvent(event)
            return True
        if t == QEvent.ShortcutOverride and event.key() >= Qt.Key_A and \
                event.key() <= Qt.Key_Z:
                    if (event.modifiers() & (self.ctrl | Qt.AltModifier | \
                            Qt.ShiftModifier)) == self.ctrl:
                        event.accept()
                        return True
        return super(Terminal, self).event(event)
    ...
    def paintRow(self, y, row, rowgfx):
        ...
        for i in range(0, self.cols):
            ...
            if self.blink and self.caretVisible:
                # TODO: Uncomment the following lines
                # and self.process.emulator.cursor.visible and \
        ...
        for i in range(0, self.cols):
            ...
            if gfx != current:
                if length > 0:
                    line = "".join(row[x:i])
                    p.setPen(fg)
                    p.drawText(x * self.font.charWidth, y * self.font.charHeight \
                            + self.font.charOffset + self.font.ascent, line)
        ...
        if not self.caretVisible:
            # TODO: Uncomment the following line
            #and self.process.emulator.cursor.y == y:
            fg, bg = self.get_colors(rowgfx[5])
            # TODO
    ...
    def paintEvent(self, event):
        ...
        self.paintRow(0, 'firedes> ' + ' '*self.cols, 'firedes> ' \
                + ' '*self.cols)
